...
export default class QuestionsDisplay extends React.Component {
    ...
    fetchData(){
        return fetch(baseUrl+"/scorecard?child_id="+this.state.childId+"&screen_type=3&plan_type=2&category_id="+this.state.data.id+"&vc="+vc, { //have to change the API call for children afterwards.
            method: "GET",
            credentials: "include",
            headers: {
    ...
    HealthQuestionAnswers() {
        fetch("http://qa.parentlane.com/api/growthcheck/ota/concerned?vc="+vc, {
            method: "POST",
            credentials: "include",
            ...
            body: JSON.stringify({
                ...
                category_id: this.state.data.id,
                concerned_area_list: this.state.selectedSymptoms,
            }),
        }).then((response) => response.json())
            .then((responseJson) => {
            ...
            <View style={{height: height}}>
                {this.state.apiData && this.state.apiData.content ?
                    ...
                    {this.state.data.id === "78a2116e-c64f-4a49-9c9a-382e5ac661b8" ?
                            ...
                            {Platform.OS === 'web' ?
                                <View style={{
                                    ...
                                    {this.state.apiData.content.overall_child_Development.selected_category.sub_category_list.map((value, index) => {
                                        return (
                                            <View style={{}} key={index}>
                                                {this.state.selectedSymptoms.includes(value.id) ?//code to be changes find an element in an array
                                                    <TouchableOpacity
                                                        onPress={() => this.onDeSelectSymptoms(this.state.apiData.content.overall_child_Development.selected_category.sub_category_list[index])}
                                                        style={{
                                                            borderWidth: 1,
                                                            borderColor: "white",
                                                            width: 300,
                                                    ...
                                                    <TouchableOpacity
                                                        onPress={() => this.onSelectSymptoms(this.state.apiData.content.overall_child_Development.selected_category.sub_category_list[index])}
                                                        style={{
                                                            borderWidth: 1,
                                                            borderColor: "white",
                                                            width: 300,
                                    ...
                                    <View style={{
                                        ...
                                        <TouchableOpacity onPress={() => this.HealthQuestionAnswers()} style={{
                                            marginTop: 5,
                                            marginLeft: 20,
                        ...
                        :
                <View style={{height: height, alignItems: "center", justifyContent: "center", backgroundColor: this.state.color, paddingTop: 20}}>
                    {Platform.OS === 'web' ?
                        <ImageWeb defaultSource={this.state.image} style={{width: 80, height: 80}}/> :
                        <Image source={this.state.image} style={{width: 80, height: 80}}/>
                    }
